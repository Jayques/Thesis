...
def plot_lp_results_fixed(malignant, benign, w, gamma, feature1, feature2, accuracy, confusion_matrix, objective_value):
    ...
    # Calculate slope-intercept form for the decision boundary
    line_formula = ""
    if w is not None and abs(w[1]) > 1e-10:
        slope = -w[0] / w[1]
        intercept = -gamma / w[1]
    ...
    # CHANGED: Title in top left with font size 16 (removed set_title)
    title_text = (f'Linear Programming Classification\n'
                  f'Accuracy: {accuracy:.1f}% | Total Slack: {objective_value:.4f}\n'
                  f'TP: {tp}, TN: {tn}, FP: {fp}, FN: {fn}')
    
    # Add title in top left corner - fontsize 16
    ax.text(0.02, 0.98, title_text, transform=ax.transAxes, fontsize=16,
            horizontalalignment='left', verticalalignment='top',
            bbox=dict(boxstyle='round,pad=0.5', facecolor='white', alpha=0.8, 
                     edgecolor='gray', linewidth=1))
    
    # CHANGED: Add boundary equation below title with font size 16
    if line_formula:
    ...
    # CHANGED: Legend in top right corner

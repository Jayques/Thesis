...
def plot_svm_results(malignant, benign, w, gamma, margin, feature1, feature2, accuracy, confusion_matrix, objective_value, C):
    ...
    # Calculate slope-intercept form for margin line equation
    line_formula = ""
    if w is not None and abs(w[1]) > 1e-10:
        slope = -w[0] / w[1]
        intercept = -gamma / w[1]
        line_formula = f'{feature2} = {slope:.3f}·{feature1} + {intercept:.3f}'
    elif w is not None:
        x_value = -gamma / w[0] if abs(w[0]) > 1e-10 else 0
        line_formula = f'{feature1} = {x_value:.3f}'
    
    # UPDATED: Title in top left corner with margin line equation - INCREASED FONT SIZE to 16
    title_text = (f'Quadratic Programming (SVM) Classification\n'
                  f'Accuracy: {accuracy:.1f}% | Margin: {margin:.4f} | C: {C}\n'
                  f'TP: {tp}, TN: {tn}, FP: {fp}, FN: {fn}')
    
    # Add title in top left corner - fontsize increased to 16
    ax.text(0.02, 0.98, title_text, transform=ax.transAxes, fontsize=16,
            horizontalalignment='left', verticalalignment='top',
            bbox=dict(boxstyle='round,pad=0.5', facecolor='white', alpha=0.8, 
                     edgecolor='gray', linewidth=1))
    
    # Add margin line equation below the title - fontsize increased to 16
    if line_formula:
        ax.text(0.02, 0.74, f'Boundary: {line_formula}', transform=ax.transAxes, fontsize=16,
                horizontalalignment='left', verticalalignment='top', style='italic',
